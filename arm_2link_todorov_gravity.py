...
class Arm(object):
    def __init__(self, m1=1.4, m2=1.1, l1=.31, l2=.27):
    ...
    def armAnglesEnd(self, endposn):
        ...
        return self.armAngles((x0,y0,x1,y1))

    def lin_sigmoid(self, x,xstart,width):
        '''linear sigmoid 0 till xstart, increases linearly to 1 for width.'''
        return np.clip(x-xstart,0,width)/width

    def evolveFns(self, q, dq, u, XY=False, dt=None):
        '''
        ...
        m1_   = self.m1    # segment mass
        m2_   = self.m2
        l1_   = self.l1   # segment length
        l2_   = self.l2
        # def __init__(self, m1=1.4, m2=1.1, l1=.31, l2=.27):
        s1_   = self.l1   # segment center of mass
        s2_   = self.l2
        i1_   = 0.0  # segment moment of inertia
        i2_   = 0.0
        ...
        for i in range(2):
            torque[i] = - H[i] + u[i] * \
                        (1 - np.float(u[i]>0)*self.lin_sigmoid(q[i],np.pi/2.,np.pi/4.) \
                            - np.float(u[i]<0)*self.lin_sigmoid(-q[i],np.pi/2.,np.pi/4) )
        ...
        if XY:
            return (self.armXY(q+dq*dt)-self.armXY(q))/dt, dvelocities
        else:
